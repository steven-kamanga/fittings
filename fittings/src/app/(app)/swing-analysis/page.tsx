...
interface SwingAnalysisResponse {
...
const Page = () => {
  ...
  const [analysisToEdit, setAnalysisToEdit] = useState<SwingAnalysis | null>(
    null
  );
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/swing-analysis/${analysisToDelete.id}`,
        {
          headers: {
  ...
  const handleStatusChange = async (analysisId: string, newStatus: string) => {
    try {
      const response = await axios.patch(
        `${process.env.NEXT_PUBLIC_API_URL}/swing-analysis/${analysisId}/${newStatus}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${(session as any)?.accessToken}`,
    ...
    try {
      const url =
        (session?.user as any)?.role === "admin"
          ? "/swing-analysis"
          : `/swing-analysis/user/${(session?.user as any)?.id}`;
      const response = await axios.get<SwingAnalysisResponse>(
